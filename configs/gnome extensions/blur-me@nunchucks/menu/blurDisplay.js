...
Signals.addSignalMethods(BlurInfo.prototype);

class RootInfo extends BlurInfo {
    _init() {
...
class BlurDeviceInfo extends BlurInfo {
    _init(kind, mount) {
        this._mount = mount;
...
class BlurVolumeInfo extends BlurInfo {
    _init(kind, volume) {
        this._volume = volume;
...
var BlurManager = class {
    ...
    _updateSpecials() {
        ...
        this._places.special.push(new BlurInfo(
            'special',
        ...
        for (let i = 0; i < dirs.length; i++) {
            ...
            try {
                info = new BlurInfo('special', file);
            } catch (e) {
    ...
    _updateMounts() {
        ...
        this._places.network.push(new BlurInfo(
            'network',
    ...
    _reloadBookmarks() {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            bookmarks.push(new BlurInfo('bookmarks', file, label));
        }
    ...
    _addMount(kind, mount) {
        ...
        try {
            devItem = new BlurDeviceInfo(kind, mount);
        } catch (e) {
            if (e.matches(Gio.IOErrorEnum, Gio.IOErrorEnum.NOT_FOUND))
                return;
            throw e;
    ...
    _addVolume(kind, volume) {
        ...
        try {
            volItem = new BlurVolumeInfo(kind, volume);
        } catch (e) {
            if (e.matches(Gio.IOErrorEnum, Gio.IOErrorEnum.NOT_FOUND))
                return;
            throw e;
...
Signals.addSignalMethods(BlurManager.prototype);
